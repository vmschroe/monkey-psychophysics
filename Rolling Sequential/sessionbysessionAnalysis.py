...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 14 20:30:54 2025
...
##Load and clean data
exec(open('/home/vmschroe/Documents/Monkey Analysis/Github/session_process.py').read())
...
def bayes_data_analysis(df, grp_name, plot_posts):
    print('----------------------------------------------------------------------')
    print('Loading data: Session ', session)
...
for session in sessions:
    dfs = [frames_ld[session], frames_ln[session], frames_rd[session], frames_rn[session]]
    group_names = ["Left hand, Distracted", "Left hand, Not distracted","Right hand, Distracted", "Right hand, Not distracted"]

    
    trace_ld, param_samps_ld, PSE_samples_ld, JND_samples_ld, ydata_ld = bayes_data_analysis(dfs[0], group_names[0], False)
    trace_ln, param_samps_ln, PSE_samples_ln, JND_samples_ln, ydata_ln = bayes_data_analysis(dfs[1], group_names[1], False)
    trace_rd, param_samps_rd, PSE_samples_rd, JND_samples_rd, ydata_rd = bayes_data_analysis(dfs[2], group_names[2], False)
    trace_rn, param_samps_rn, PSE_samples_rn, JND_samples_rn, ydata_rn = bayes_data_analysis(dfs[3], group_names[3], False)
    
    
    ############
    
    plot_curves_tog(trace_ld, param_samps_ld, ydata_ld, group_names[0], trace_ln, param_samps_ln, ydata_ln, group_names[1], True, "Left Hand, Distractor = "+str(distAMPS[session])+" microns")
    plot_curves_tog(trace_rd, param_samps_rd, ydata_rd, group_names[2], trace_rn, param_samps_rn, ydata_rn, group_names[3], True, "Right Hand, Distractor = "+str(distAMPS[session])+" microns")
